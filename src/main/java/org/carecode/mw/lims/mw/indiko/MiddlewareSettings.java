package org.carecode.mw.lims.mw.indiko;

/**
 *
 * @author Dr M H B Ariyaratne, buddhika.ari@gmail.com
 * 
 */
public class MiddlewareSettings {
    private AnalyzerDetails analyzerDetails;
    private LimsSettings limsSettings;

    public AnalyzerDetails getAnalyzerDetails() {
        return analyzerDetails;
    }

    public void setAnalyzerDetails(AnalyzerDetails analyzerDetails) {
        this.analyzerDetails = analyzerDetails;
    }

    public LimsSettings getLimsSettings() {
        return limsSettings;
    }

    public void setLimsSettings(LimsSettings limsSettings) {
        this.limsSettings = limsSettings;
    }

    
    
    
}



